...
- **DIAG-3.19-23** 🟡 **Top-level diagram `sources` participate in autosave dirty fingerprint + history undo/redo (MVP-4a)** — history undo wiring is now pinned by `useDiagramHistoryStore.test.ts` (`DIAG-3.19-23`): editing top-level `sources` is captured in the history snapshot, and undo restores the prior list via `dispatch.loadDoc`. Autosave-dirty-fingerprint and on-disk draft assertions for top-level sources remain a follow-up.
