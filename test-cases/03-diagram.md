...
- **DIAG-3.13-50** ✅ **Mounted in NodeProperties / LineProperties / FlowProperties / DiagramProperties** — `AttachmentsSection` is rendered at the bottom of all 4 entity-scoped panels with the right `EntityAttachmentTarget` (`node | connection | flow | root`); LayerProperties is excluded because `'layer'` is not in the union. Pinned by `PropertiesPanel.test.tsx` (`DIAG-3.13-50`): each entity selection routes the panel-mounted Attach button through `onOpenDocPicker(<scope>, id)` with the correct scope and id. (For `flow` selection, both the flow-scoped and the root-scoped AttachmentsSection render together, and the test asserts the flow-scoped one fires first.)
...
- **DIAG-3.19-23** ✅ **Top-level diagram `sources` participate in autosave dirty fingerprint + history undo/redo (MVP-4a)** — pinned by two tests: `useDiagramHistoryStore.test.ts` (`DIAG-3.19-23`, history snapshot + undo restore via `dispatch.loadDoc`) and `useDiagramPersistence.test.tsx` (`DIAG-3.19-23`, dirty bit flips when `sources` diverges from the load snapshot and clears when reverted).
