...
- **DOC-4.2-06** ✅ **Task-list checkbox markdown round-trips through HTML and back unchanged** — re-scoped from the original "checkbox toggle updates markdown" wording, which asserted a behaviour that does not exist (`markdownToHtml` emits a `disabled` `<input type="checkbox">` rather than a Tiptap `taskItem`, so in-editor clicks cannot toggle the box). The actually-shipped invariant is the on-disk shape: `- [ ] foo` and `- [x] foo` both round-trip cleanly through `markdownToHtml` → `htmlToMarkdown`. Vitest in `src/app/knowledge_base/features/document/extensions/markdownSerializer.test.ts` (`DOC-4.2-06: task-list checkbox markdown round-trips …`). Adopting the markdown-it task-list plugin (which would change the persisted disk shape) is intentionally out of scope.
...
- **DOC-4.3-38** ✅ **Enter in rawBlock splits with smart list-item handling.** Vitest in `src/app/knowledge_base/features/document/components/MarkdownEditor.tiptapEditorRef.test.tsx`. The list-item branch of the markdownReveal Enter keymap (markdownReveal.ts:125–225) splits `<li><rawBlock>**bold** trailing text</rawBlock></li>` at offset 8 into two list items; assertions cover the split shape and per-item text content.
- **DOC-4.3-39** ✅ **Backspace at rawBlock start merges with previous block's rightmost textblock.** Vitest in `src/app/knowledge_base/features/document/components/MarkdownEditor.tiptapEditorRef.test.tsx`. The Backspace branch of the markdownReveal keymap (markdownReveal.ts:257–319) deletes the rawBlock and inserts its literal markdown content at the end of the previous textblock; assertions cover the merged text and single-block invariant.
...
- **DOC-4.5-13** ✅ **Force-exit rawBlock before structural commands.** Vitest in `src/app/knowledge_base/features/document/components/MarkdownEditor.tiptapEditorRef.test.tsx`. With the cursor inside a rawBlock, calling `forceExitRawBlock(editor)` (rawSyntaxEngine.ts:417) returns `true` and converts the rawBlock back to rich content; the subsequent `chain().focus().toggleBulletList().run()` then wraps the result in a `bulletList` — the same two-step sequence the toolbar's bullet-list button uses (`MarkdownToolbar.tsx:131`).
...
- **DOC-4.5-18** ✅ **Link button with text selected wraps selection.** Vitest in `src/app/knowledge_base/features/document/components/MarkdownEditor.tiptapEditorRef.test.tsx`. With a multi-character `TextSelection` covering "anchor" in "see anchor for context", the toolbar Link chain `editor.chain().focus().extendMarkRange("link").setLink({ href }).run()` applies a `link` mark to exactly that text run; assertions verify the marked text and href.
