...
- **TAB-11.2-08** ✅ **External file change while pane is open triggers `ConflictBanner`** — new `useTabFileWatcher` hook (mirroring `useDocumentFileWatcher` / `useDiagramFileWatcher`) subscribes to `vault_change` via the existing `FileWatcherContext`. When disk content changes and the pane is dirty, `conflictContent` is set and `TabView` mounts `ConflictBanner` with reload / keep handlers; when clean, `useTabContent.refresh()` runs silently and toasts. `useTabContent` was extended minimally to track `diskChecksumRef` (updated on load + flush) so self-writes don't trigger a false-positive banner. Closes Theme C. _(unit: `useTabFileWatcher.test.tsx` — five branches mirroring DOC-4.15-01..05.)_
...
- **TAB-11.4-06** ✅ **End-to-end import flow** — `e2e/tab_gp_import.spec.ts` drives the palette command, sends bytes via Playwright's built-in `filechooser` event, and asserts the resulting `.alphatex` opens in a tab pane (mirrors the `tab-view-canvas` + `tab-view-engine-error` pair from TAB-11.2-14). Production uses a hidden `<input type="file">` + `input.click()` (NOT `showOpenFilePicker`), so the original "needs a custom file-picker mock" premise was wrong — Playwright handles it natively, no harness code added. To avoid bundling a real Guitar Pro fixture, the test sends alphaTex bytes named `tune.gp7`; alphaTab's `ScoreLoader.buildImporters()` falls through GP3-5 / Gpx / Gp7-8 / MusicXml / Capella → AlphaTexImporter. _(e2e: `tab_gp_import.spec.ts`.)_
