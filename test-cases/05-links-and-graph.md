...
- **LINK-5.1-10** ✅ **Rename into a different folder** — `e2e/explorer_move_item.spec.ts` seeds the `with_links` fixture (`b.md` references `[[a]]`), creates a `subfolder/` via node `fs`, and calls a NODE_ENV-gated `__kbE2EMoveItem(sourcePath, targetFolderPath)` window seam (set in `KnowledgeBaseInner` only when `process.env.NODE_ENV !== "production"` — same gate pattern as `ServiceWorkerRegister`; production builds drop the assignment via dead-code elimination). The seam invokes `fileExplorer.moveItem` + `propagateMoveLinks` directly, mirroring production's chain minus the `diagramBridgeRef` indirection (the bridge's `handleMoveItem` is itself a passthrough). Asserts via node-side `fs.readFile` that `subfolder/a.md` exists, root no longer has `a.md`, and `b.md` rewrote `[[a]]` to `[[subfolder/a]]`. Bypasses the headless-Chromium `dataTransfer.getData()` gating that blocks synthetic DnD drops. _(e2e: `explorer_move_item.spec.ts`.)_
...
- **LINK-5.1-12** ✅ **Atomic rename — no lost-reference window after completion** — `propagateRename` updates the link-index first via `renameDocumentInIndex`, then walks the pre-rename `getBacklinksFor(oldPath)` snapshot to rewrite each source file's wiki-link content. After completion the index points to the new path and every backlink file contains the new path; no in-memory or on-disk reference to the old path remains. Vitest in `src/app/knowledge_base/shared/hooks/useFileExplorer.helpers.test.ts` (`LINK-5.1-12: rename leaves no lost-reference window …`).
...
- **LINK-5.2-03** 🧪 **Deleted doc's links become red pills** — other docs referencing it via `[[x]]` now show "click to create" state. _(e2e: `e2e/wiki_link_red_pill_on_delete.spec.ts`. Explorer-delete goes through `fileExplorer.deleteFile` + `linkManager.removeDocumentFromIndex` — no on-disk strip — and the wikiLink extension reads `existingDocPaths` through a getter chained off a host-owned ref, so the host calls `forceRepaint` after the index shrinks and every live NodeView re-paints its blue/red pill in place. The MVP-5 "disable strip / wire watcher reload" diagnosis applied to the cascade-delete path in `deleteDocumentWithCleanup`, not the explorer-delete path tested here.)_
...
- **LINK-5.4-03** 🧪 **Delete in explorer removes backlinks in open docs** — doc A open with a link to deleted B → A's pill flips to red. _(e2e: `e2e/wiki_link_red_pill_on_delete.spec.ts`. Same behaviour as LINK-5.2-03 from the open-editor side; the explorer-delete path leaves b.md untouched on disk and relies on the wikiLink extension's getter+forceRepaint hookup to flip the pill without a pane reload.)_
