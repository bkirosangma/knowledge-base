...
- **SHELL-1.13-03** 🧪 **Theme persists across reload via vault config** — pre-seeding `.archdesigner/config.json` with `{ theme: "dark" }` and opening the folder applies dark on first mount before any user action. _(e2e: `e2e/themeToggle.spec.ts`)_
