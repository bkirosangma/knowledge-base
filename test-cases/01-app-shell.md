...
- **SHELL-1.15-03** ❌ **`themeColor` lives in viewport export (Next 16)** — moved out of `metadata` to satisfy Next 16's metadata classifier; `npm run build` is silent. _(MVP-5 follow-up: Next 16 metadata-classifier check — build-time, needs unit test against `app/layout.tsx` exports)_
