...
- **SHELL-1.16-01** ❌ **Tabbing to an icon button surfaces the tooltip with no OS delay** — keyboard-focusing any button wrapped in `<Tooltip>` (e.g. the diagram toolbar zoom controls) displays its `[role="tooltip"]` bubble immediately; the `:has(:focus-visible)` rule fires the same frame focus moves. _(visual; verified manually per ticket Verify block)_
- **SHELL-1.16-02** ❌ **Hover surfaces the same bubble** — moving the pointer over the trigger displays the bubble; moving away hides it. Same DOM node, no delay. _(visual)_
