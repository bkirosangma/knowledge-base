...
describe('classifyError', () => {
  it('FS-2.3-01: is idempotent on an existing FileSystemError', () => {
    const err = new FileSystemError('malformed', 'bad json')
    expect(classifyError(err)).toBe(err)
  })

  it('FS-2.3-02: maps DOMException NotFoundError → not-found', () => {
  ...
  it('FS-2.3-03: maps NotAllowedError / SecurityError → permission', () => {
...
describe('readOrNull', () => {
  ...
  it('FS-2.3-06: returns null on not-found', async () => {
    expect(await readOrNull(async () => {
      throw new FileSystemError('not-found', 'gone')
    })).toBeNull()
  })

  it('FS-2.3-07: re-throws permission / malformed / quota / unknown unchanged', async () => {
    const err = new FileSystemError('permission', 'denied')
    await expect(readOrNull(async () => { throw err })).rejects.toBe(err)
  })

  it('FS-2.3-07: classifies raw DOMException-like throws before re-throwing', async () => {
    let caught: unknown
    try {
