...
export default function DocumentProperties({
  ...
  if (collapsed) {
    return (
      <div className="flex-shrink-0 bg-surface border-l border-line flex flex-col overflow-hidden" style={{ width: 36 }}>
  ...
  if (!filePath) {
    return (
      <div className="flex-shrink-0 bg-surface border-l border-line flex flex-col overflow-hidden" style={{ width: 280 }}>
        {onToggleCollapse ? (
        ...
        <div className="p-4 text-sm text-mute">No document selected</div>
      </div>
    );
  }
    ...
    <div className="flex-shrink-0 bg-surface border-l border-line flex flex-col overflow-hidden" style={{ width: 280 }}>
      ...
      {onToggleCollapse ? (
        <Tooltip label="Collapse properties">
          ...
          >
            <span className="text-xs font-bold text-ink-2 uppercase tracking-wider">Properties</span>
            <span className="text-xs text-mute truncate">{filename}</span>
            <ChevronRight size={14} className="ml-auto text-mute flex-shrink-0" />
          </button>
        </Tooltip>
      ) : (
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-line">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-3 border-b border-line">
          <div className="text-xs font-semibold text-mute uppercase tracking-wider mb-2">Stats</div>
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="flex items-center gap-1.5 text-ink-2">
              <Hash size={12} className="text-mute" />
          ...
          {outbound?.length ? (
            <div className="space-y-1">
              {outbound.map((link, i) => (
                <button
                  key={i}
                  className="flex items-center gap-1.5 text-xs text-accent hover:text-accent-2 hover:underline w-full text-left"
            ...
            <div className="text-xs text-mute">No outbound links</div>
          )}
        </div>
        ...
        <div className="px-4 py-3 border-b border-line">
          ...
          {backlinks.length > 0 ? (
            ...
            <div className="text-xs text-mute">No backlinks</div>
          )}
        </div>
