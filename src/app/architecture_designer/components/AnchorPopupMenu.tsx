...
export default function AnchorPopupMenu({
  ...
  return (
    <div ref={menuRef} onMouseEnter={onMenuEnter} onMouseLeave={onMenuLeave}>
      ...
      {subPanel === "element" && (
        <div className="fixed z-[9999] bg-white rounded-lg shadow-lg border border-slate-200 py-1 w-[220px] max-h-[300px] flex flex-col" style={subPanelStyle}>
          ...
          <div className="overflow-y-auto flex-1">
            {filteredNodes.length === 0 && (
              <div className="px-3 py-2 text-[12px] text-slate-400">No elements found</div>
            )}
            {filteredNodes.map((n) => (
              <button
                key={n.id}
        ...
        <div className="fixed z-[9999] bg-white rounded-lg shadow-lg border border-slate-200 py-1 w-[220px] max-h-[300px] flex flex-col" style={subPanelStyle}>
          ...
          <div className="overflow-y-auto flex-1">
            {filteredTypes.length === 0 && (
              <div className="px-3 py-2 text-[12px] text-slate-400">No types defined</div>
            )}
            {filteredTypes.map((t) => (
              <button
                key={t}
