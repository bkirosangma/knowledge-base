...
export function useSelectionRect({
  toCanvasCoords, isBlocked, nodes, regions, getNodeDimensions, setSelection, pendingSelectionRef,
  ...
  const begin = useCallback((e: React.MouseEvent) => {
    ...
    const resolveRect = (ev: MouseEvent) => {
      ...
      const rectResult = resolveRectangleSelection(rect, nodesRef.current, regionsRef.current, getNodeDimensions);
      if (isToggle) {
