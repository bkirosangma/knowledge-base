...
(GitHub Pages workflow removal, FSA infrastructure deletion, and CI on `macos-latest` are deferred to MVP-1d. File watching landed in MVP-1b; settings persistence, vault switcher UI, and the uninitialized-folder splash landed in MVP-1c.)
...
- ✅ **Manifest reference in layout** — `src/app/layout.tsx` builds `metadata.manifest` and `metadata.icons` from `process.env.NEXT_PUBLIC_BASE_PATH` so absolute hrefs are correct under both root and GH-Pages basePath deployments. Next 16 requires `themeColor` on the `viewport` export (not `metadata`), so we expose both: `metadata` carries the manifest + icons, `viewport` carries `themeColor`. The web manifest itself uses relative URLs (`./`, `./icon.svg`) so it resolves correctly under any basePath.
- ⚙️ **Service worker (`public/sw.js`)** — hand-rolled (next-pwa is not Next-16 compatible). Self-locates from `self.location.pathname` so all lane checks and precache entries adapt to the deployment basePath at runtime (root on dev/Vercel, `/knowledge-base` on GitHub Pages). Three cache lanes: (1) `<base>/__kb-cache/*` is cache-only against `kb-files-v1` (vault-content cache populated by `useOfflineCache`); (2) `<base>/_next/static/*` is cache-first against `kb-static-v3` (content-hashed bundles, KB-044); (3) navigation requests are network-first with the response stored at `<base>/` and offline fallback to that cached shell, so DevTools "Offline" reloads boot the app instead of Chrome's offline page. Manifest and icon are cache-first; default lane is network-first with cache fallback. Install precaches `[<base>/, <base>/index.html, <base>/manifest.json, <base>/icon.svg, <base>/soundfonts/sonivox.sf2]` (best-effort — entries that 404 don't abort install). Activate hook drops old static caches but preserves the file cache.
...
- ⚙️ **GitHub Pages deploy** (`.github/workflows/pages.yml`) — on every push to `main` (and on `workflow_dispatch`), builds a static export with `GITHUB_PAGES=true npm run build` and publishes `out/` via `actions/upload-pages-artifact@v3` + `actions/deploy-pages@v4`. Concurrency group `pages` serialises deploys without cancelling an in-flight upload. Tests are not re-run here — `ci.yml` is the single gate of correctness.
- ⚙️ **Static-export configuration** (`next.config.ts`) — when `GITHUB_PAGES=true`, Next emits a static export under `/out` with `basePath: "/knowledge-base"`, `trailingSlash: true`, and `images.unoptimized: true`. Local dev and Vercel deploys are unaffected (config is gated). `NEXT_PUBLIC_BASE_PATH` is exposed to client code so absolute asset URLs (manifest, icons, sw.js, sample-vault, soundfont) prefix correctly. The service worker self-locates from `self.location.pathname` and works under any basePath without a build flag. The web manifest uses relative URLs (`./`, `./icon.svg`) so it resolves correctly under either deployment.
