...
- ✅ **DocPreviewModal** — universal read-only document preview triggered by clicking any attached doc or wiki-link backlink in any entity panel. Blurs the diagram canvas (`blur-sm pointer-events-none`) and disables interactions while open. Header shows filename, "Read only" chip, optional entity name badge, "Open in pane" button, and close ✕. Body renders document content via `markdownToHtml()` in `.markdown-editor .ProseMirror` — pixel-identical to the doc pane. Wiki-links render as styled pills: the DOMPurify config allowlists `data-wiki-link` and `data-wiki-section` so the link identity survives sanitization, and `prose.css` provides a non-editor-scoped `.wiki-link` pill rule so the visual matches the editor without depending on the contenteditable selector. Wiki-links are clickable: a delegated click handler on the body resolves the link target via `resolveWikiLinkPath(linkPath, currentDocDir)` (current dir derived from the previewed doc's path), then forwards to `onOpenInPane` and closes the modal — so previewing a doc, clicking a wiki-link inside it, and continuing reading the target in the pane is one motion. Rendered via `ReactDOM.createPortal` at `document.body`, unaffected by ancestor `filter`/`transform`. Closes on Escape or backdrop click. HTML output sanitized with a DOM-based sanitizer before render.
