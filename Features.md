...
### 2.1 Folder Picker
`shared/hooks/useFileExplorer.ts`, `shared/hooks/fileExplorerHelpers.ts`, `shared/utils/directoryScope.ts`, `types/file-system.d.ts`. Phase 1.5 (2026-04-18) moved pure module-level helpers (isSupported, isDiagramData, uniqueName, collectFilePaths, resolveParentHandle, findChildren, copyDirContents, readTextFile, writeTextFile, getSubdirectoryHandle) into `fileExplorerHelpers.ts`. The hook itself stayed composed because its CRUD / draft / handle operations share state tightly; finer decomposition is deferred to Phase 3 (DIP / repository pattern). useFileExplorer.ts: 675 → 557 lines.
