...
- ✅ **Vault switcher dropdown (MVP-1c)** — left-side trigger button in the header showing the current vault basename (or "No vault open" when none); `data-testid="vault-switcher-trigger"` opens a dropdown menu with three actions: **Open Vault…** (drives `tauri-plugin-dialog::pickFolder` then `useFileExplorer.switchVault(path)` with dirty-confirm), **Recent Vaults** (MRU 5 from `settingsStore.recents`; selecting an entry calls `switchVault(path)`), and **Initialize Vault…** (creates `.archdesigner/config.json` in the current folder via `vaultConfig.init`). Closes on outside-click, Escape, or after an action. (`src/app/knowledge_base/shared/components/VaultSwitcher.tsx`)
...
### 1.15 PWA — Manifest, Service Worker (Phase 3 PR 3)
`public/manifest.json`, `public/sw.js`, `public/icon.svg`, `shell/ServiceWorkerRegister.tsx`
...
- ✅ **Manifest reference in layout** — `src/app/layout.tsx` builds `metadata.manifest` and `metadata.icons` from `process.env.NEXT_PUBLIC_BASE_PATH` so absolute hrefs are correct. Next 16 requires `themeColor` on the `viewport` export (not `metadata`), so we expose both: `metadata` carries the manifest + icons, `viewport` carries `themeColor`. The web manifest itself uses relative URLs (`./`, `./icon.svg`) so it resolves correctly under any basePath.
- ⚙️ **Service worker (`public/sw.js`)** — hand-rolled (next-pwa is not Next-16 compatible). Self-locates from `self.location.pathname` so all lane checks and precache entries adapt to the deployment basePath at runtime. Two cache lanes: (1) `<base>/_next/static/*` is cache-first against `kb-static-v3` (content-hashed bundles, KB-044); (2) navigation requests are network-first with the response stored at `<base>/` and offline fallback to that cached shell. Manifest and icon are cache-first; default lane is network-first with cache fallback. Install precaches `[<base>/, <base>/index.html, <base>/manifest.json, <base>/icon.svg, <base>/soundfonts/sonivox.sf2]` (best-effort). Activate hook drops old static caches.
- ⚙️ **`ServiceWorkerRegister` component** — renders inside `KnowledgeBaseInner`. Calls `navigator.serviceWorker.register(`${basePath}/sw.js`)` (basePath sourced from `NEXT_PUBLIC_BASE_PATH`) only when `process.env.NODE_ENV === "production"` so dev mode / Turbopack HMR isn't intercepted.
...
- ⚙️ **`PaneEntry.anchor`** (`shell/PaneManager.tsx`) — per-pane wiki-link anchor target threaded through `panes.openFile(path, fileType, { anchor })`. Each pane carries its own anchor so left/right navigation never crosses streams; `openFile` without `{ anchor }` resets the entry's anchor to `null` so a stale `#section` cannot persist across navigations. Stripped at the `SavedPaneEntry` boundary so it does not survive reload. Wiki-link MVP 3 plumbing for `[[doc.md#section]]`.
...
- ⚙️ **FileWatcherContext** (`shared/context/FileWatcherContext.tsx`) — event-driven file-watcher context backed by the Rust `notify`-debouncer-full watcher (MVP-1b). Listens for `vault_change` Tauri events (`{ kind, path, oldPath? }` with POSIX-relative paths) and dispatches them to named subscribers; `refresh()` still fires all subscribers immediately. Accepts a `vaultPath: string | null` prop; calls `tauriBridge.watchStart(vaultPath)` on mount and `watchStop` on unmount / path change (200 ms debounce coalesce window on the Rust side). Exposes `lastSyncedAt` (epoch ms, reset on each event) so the Footer can render a "Last synced N s ago" heartbeat chip. Polling timers (5 s / 30 s) and idle/visibility/input backoff are retired. See `src-tauri/src/vault/watcher.rs` for the Rust side.
- ⚙️ **Watcher `Modified`→`Deleted` post-process (MVP-1c)** — `src-tauri/src/vault/watcher.rs::postprocess_existence` runs in the dispatcher worker before each `vault_change` is forwarded: when the kind is `Modified` and the absolute path no longer exists on disk, the kind is rewritten to `Deleted`. Closes half of the macOS FSEvents kind-mapping gap discovered in MVP-1b (`tokio::fs::remove_file` was emitting `Modified(Data)` instead of `Deleted` on macOS). The rename-cookie half stays deferred to MVP-4.
...
- ✅ **BrokenAnchorBanner** (`shared/components/BrokenAnchorBanner.tsx`) — shell-level amber banner shown after a save deletes one or more headings that other docs link to (`useLinkIndex` exposes `brokenAnchorState`). Reports `N heading(s) removed from <docPath>` and `M wiki-link(s) now broken`. "Remove anchors" rewrites every consuming source markdown via `stripWikiLinkAnchors` (drops the `#section` while preserving path + alias) then dismisses; "Leave broken" only dismisses. Mounted in `knowledgeBase.tsx` between Header and the explorer/viewport row; hidden when `affectedRefs` is empty (silent deletion of an unreferenced heading).
...
### 2.1 Folder Picker
`shared/hooks/useFileExplorer.ts`, `shared/hooks/fileExplorerHelpers.ts`, `shared/hooks/useDrafts.ts`, `shared/utils/directoryScope.ts`. Phase 1.5 (2026-04-18) moved pure helpers into `fileExplorerHelpers.ts`. Phase 1.5b (2026-04-18) extracted `useDrafts` (dirtyFiles Set + refreshDrafts / removeDraft / markDirty) and the now-deleted `useDirectoryHandle` (retired in MVP-1d; its IndexedDB handle persistence path is obsolete now that Tauri owns the vault path via `settingsStore`). useFileExplorer.ts is the composition root that owns the tree / activeFile / CRUD ops, wired to Tauri's native vault path.
...
- ✅ **Uninitialized-vault splash (MVP-1c)** — when `vaultStatus === "uninitialized"` (folder picked but no `.archdesigner/config.json`), `KnowledgeBaseInner` renders `UninitializedVaultSplash` in place of the explorer + panes; the splash blocks the app interior, explains that the chosen folder is not a vault, and exposes an **Initialize Vault** button that calls `vaultConfigRepo.init(name)` then re-evaluates `vaultStatus` so the splash dismisses on success. Also offers **Open a different folder** (re-runs `vault_pick`). (`src/app/knowledge_base/shared/components/UninitializedVaultSplash.tsx`, init-guard wired in `src/app/knowledge_base/knowledgeBase.tsx`)
- ⚙️ **Vault path persistence via `tauri-plugin-store` (MVP-1c)** — last-opened vault path and the MRU-5 recents list persist across launches in a Tauri-managed JSON store (default location: `tauri::path::app_config_dir() + "settings.json"`). The Rust side (`src-tauri/src/settings/{mod,store,commands}.rs`) registers `tauri-plugin-store` and exposes typed `Settings` plus `settings_get` / `settings_set` commands. The TS facade (`src/app/knowledge_base/infrastructure/settingsStore.ts`) wraps `invoke()` with `getSettings`, `setLastPath`, `clearLastPath`, `pushRecent` (dedup + cap at 5, MRU-first), `getRecents`, and `setClaudeChatHeight`. On launch, `useFileExplorer` reads `settingsStore.lastPath`; on a successful `vault_pick` or `switchVault(path)`, both `setLastPath` and `pushRecent` fire so the next launch auto-restores and the switcher's recents stay current.
...
- ✅ **Flow start/end and order highlighting** — when a flow is focused (selected or hovered), nodes listed in `startNodeIds` glow green with a "Start" pill; nodes in `endNodeIds` glow red with an "End" pill; nodes absent from both lists render no glow or pill. Nodes present in `nodeOrders` display a blue corner numeral (top-left); in lock+edit mode the numeral becomes a dashed editable input. Connection labels outside the flow are hidden. Non-color signals (pills, numerals) preserve WCAG 1.4.1 (KB-032). Implemented in `DiagramView.tsx` (`flowOrderData` memo), `components/DiagramNodeLayer.tsx`, `components/OrderBadge.tsx`, `components/Element.tsx`, `components/ConditionElement.tsx`.
- ✅ **Lock into Flow** — activating lock (FlowProperties button or `Cmd/Ctrl+L` while a flow is selected) enters a session-only focus mode: non-member nodes and connections render dimmed with `pointer-events: none`; clicking the empty canvas does not deselect the flow; clicking a member node stacks `<FlowProperties>` on top and `<NodeProperties>` below in the Properties panel; a `<LockBanner>` across the canvas top names the locked flow and offers an Unlock button. Pressing `Cmd/Ctrl+L` again, navigating to a different file, or clicking Unlock all exit lock mode. `state/DiagramInteractionContext.tsx` (`lockedFlowId` slice), `components/LockBanner.tsx`, `properties/PropertiesPanel.tsx` (stack mode).
- ✅ **Document attachment** — attach existing docs to a flow from FlowProperties; create & attach a new blank doc (with optional "Edit now" to open in pane); detach with optional cascade delete that strips wiki-links from referencing docs and shows a deduplicated reference list before confirming. The MVP-2b Create-&-Attach picker (`CreateAttachEntityModal`) renders a 4-tab type strip (`Document | Diagram | SVG | Tab`); only the Document tab persists today — non-doc tabs disable Confirm with a deferred-persistence notice so the UI contract is ready when the data layer widens. `features/diagram/properties/FlowProperties.tsx`, `features/diagram/components/CreateAttachEntityModal.tsx`, `features/diagram/components/DetachDocModal.tsx`
...
- ✅ **FlowProperties** — name, category, member connections, delete, document attachment (attach existing, create & attach, detach with optional cascade delete); all attach/detach operations are recorded in the action history and are undoable/redoable.
  - ✅ **Member nodes section** — lists every node touched by the flow's connections with an editable per-node order field, Start/End checkboxes, and a "Number sequentially" button that assigns ascending integers. Lock-into-Flow button (`Cmd/Ctrl+L`) appears here and in the toolbar.
...
- ✅ **AttachmentsSection** (`properties/AttachmentsSection.tsx`) — 4-way (`docs | diagrams | svgs | tabs`) attachment list rendered at the bottom of every entity-scoped properties panel. Reads buckets via the controller's `attachmentsByType(target)` selector; Preview opens the unified `AttachmentPreviewModal` via `openAttachmentPreviewFor(target)` (seeded with all the entity's attachments); Detach forwards to the existing detach handler; Attach opens `CreateAttachEntityModal` via `onOpenDocPicker(entityType, entityId)`. Mounted in `NodeProperties`, `LineProperties`, `FlowProperties`, and `DiagramProperties` (target = `'root'` keyed by the diagram filename). `LayerProperties` is excluded — `'layer'` is not in the `EntityAttachmentTarget` union. MVP-2b only ever populates the docs bucket; the other three are reserved for future SVG / Tab / Diagram-source MVPs.
- ✅ **Source links — diagram entities (MVP-4a)** — every diagram entity (top-level diagram, node, connection, layer, flow) carries an optional `sources?: SourceLink[]` editable in its Properties panel via the shared `<SourcesSection>`. URL whitelist is `http`/`https`; rows have an inline title and a one-click "Open" affordance (`target="_blank" rel="noopener noreferrer"`). Read-only mode hides Add and Remove. Persists through diagram JSON. (`shared/types/sources.ts`, `shared/components/SourcesSection.tsx`, `features/diagram/properties/{NodeProperties,LineProperties,LayerProperties,FlowProperties,DiagramProperties}.tsx`, `shared/utils/persistence.ts`)
- ⚙️ **Top-level diagram `sources` flow through the document state slice + history** (MVP-4a) — `useDiagramDocument` owns `sources` state; the `DiagramDoc` snapshot exposed to `useDiagramHistoryStore` includes `sources` so undo/redo restores the prior list, and `useDiagramPersistence` hashes `sources` into the autosave fingerprint so edits dirty the file and write through to the draft. (`features/diagram/hooks/useDiagramDocument.ts`, `features/diagram/hooks/useDiagramHistoryStore.ts`, `features/diagram/hooks/useDiagramPersistence.ts`, `features/diagram/hooks/useDiagramController.ts`)
...
### 3.20 Attachment Preview Modal
`diagram/components/AttachmentPreviewModal.tsx`
- ✅ **AttachmentPreviewModal** — universal read-only attachment preview triggered by clicking any attached doc or wiki-link backlink in any entity panel. MVP-2b widens the contract to a 4-way `PreviewItem[]` shape (`type: 'document' | 'diagram' | 'svg' | 'tab'`, `filename`, optional `title`/`entityName`); when more than one item is supplied the modal renders a left rail grouped by type so the user can switch between attached items without closing. Body content is dispatched on `item.type`: `document` renders real markdown content via `markdownToHtml()` inside `.markdown-editor .ProseMirror`; `diagram`, `svg`, and `tab` show a "Preview not yet implemented — use Open in pane to view" placeholder (production data flow only emits document items in MVP-2b). Blurs the diagram canvas (`blur-sm pointer-events-none`) while open. Header shows the active item's title (or filename), "read only" chip, optional entity-name badge, "Open in pane" button, and close ✕. Wiki-links inside a document body render as styled pills: the DOMPurify config allowlists `data-wiki-link` and `data-wiki-section` so the link identity survives sanitization, and `prose.css`'s `.markdown-editor .wiki-link` pill rule supplies the visual. A delegated body click handler resolves the link target via the host-injected `resolveWikiLinkPath(linkPath, currentDocDir)`, forwards `(filename, anchor)` through `onOpenInPane`, and closes the modal — so previewing a doc, clicking a wiki-link inside it, and continuing reading the target in the pane is one motion. Rendered via `ReactDOM.createPortal` at `document.body`, unaffected by ancestor `filter`/`transform`. Closes on Escape (via `useFocusTrap`) or backdrop click. Resizable shell (480px–90vw) with edge-drag handles. HTML output sanitized with DOMPurify before render.

### 3.21 Diagram File Watcher
`features/diagram/hooks/useDiagramFileWatcher.ts`
- ⚙️ **`useDiagramFileWatcher`** — subscribes to the `"content:diagram"` `vault_change` event tick; compares `diskChecksumRef` to the current on-disk checksum on each event. If the file changed and the diagram is clean, silently reloads (records a "Reloaded from disk" history entry, moves the saved point, shows a toast). If the file changed and the diagram is dirty, sets `conflictSnapshot` so `DiagramView` can show a `ConflictBanner`; `handleKeepEdits` suppresses re-prompting for the same disk version via `dismissedChecksumRef`. Exposes `conflictSnapshot`, `handleReloadFromDisk`, and `handleKeepEdits`. Wired into `DiagramView` via `ConflictBanner`.
...
- ⚙️ **SluggedHeading** (`MarkdownEditor.tsx`) — Tiptap extension that replaces StarterKit's bundled Heading; stamps `data-heading-id` and `id` on every rendered heading via both `renderHTML` (read-mode + serialised output) and a React NodeView (live edit-mode). Slug values come from canonical `headerSlug(text)` so the in-doc heading id, the link-index `headers[].id`, and any `[[doc#section]]` reference all agree. The NodeView also mounts a `HeadingCopyLink` button as a sibling to the heading content when `currentDocFilename` is supplied.
- ✅ **HeadingCopyLink** (`components/HeadingCopyLink.tsx`) — hover-revealed (opacity-0 → group-hover:opacity-100) icon button mounted next to every rendered heading via the `SluggedHeading` NodeView. Click copies `[[<currentDocFilename>#<header-slug>]]` to the clipboard (with `execCommand` fallback) and flips to `Copied!` for ~1500 ms before reverting. Hidden when `currentDocFilename` is missing; `data-testid="heading-copy-link-<slug>"` for testing.
...
- ⚙️ **`stripWikiLinkAnchors(markdown, targetPath, deletedIds[])`** — strips `#section` from wiki-links targeting `targetPath` whose anchor is in `deletedIds`; preserves the link path and any alias. Used by the BrokenAnchorBanner "Remove anchors" action to rewrite consuming source markdown after a heading delete. `features/document/utils/wikiLinkParser.ts`
- ⚙️ **`updateWikiLinkAnchors(markdown, targetPath, renames)`** — rewrites `[[targetPath#oldSlug]]` → `[[targetPath#newSlug]]` for every `oldSlug → newSlug` entry in `renames`; preserves the link path and any alias text. Used by the header rename auto-refactor (`useLinkIndex.updateDocumentLinks`) to walk every consuming source markdown on disk so a heading rename on one doc updates references across the vault in one save. `features/document/utils/wikiLinkParser.ts`
...
- ✅ **Source links — documents (MVP-4a)** — `DocumentProperties` mounts the shared `<SourcesSection>` so every `.md` document carries an optional `sources?: SourceLink[]`. Edits flow through `onUpdateSources` into `useDocumentContent`, which persists the list in YAML frontmatter; read-only mode hides Add and Remove. (`shared/types/sources.ts`, `shared/components/SourcesSection.tsx`, `features/document/properties/DocumentProperties.tsx`)
...
- ⚙️ **`LinkIndexEntry.headers`** (`features/document/types.ts`) — per-doc `HeaderInfo[]` (`{ id, text, level }`) populated by `extractHeaders(content)` on every save. Used by the header rename / delete auto-refactor to diff prior vs current heading sets via `findHeaderRename`.
- ⚙️ **`findHeaderRename(prev, next)`** (`hooks/useLinkIndex.ts`) — diff helper: when `prev` has exactly one removed and exactly one added heading at the same level (or with the same text) it classifies the change as a rename and returns `{ from, to }`. Otherwise returns `null` (treats multi-removed / multi-added / additions-only as deletions or no-op). Drives the rename auto-refactor.
- ⚙️ **Header rename auto-refactor** — when `updateDocumentLinks` detects a rename via `findHeaderRename`, it (a) rewrites the in-memory link-index `sectionLinks` entries pointing at the renamed heading and (b) walks every consuming source markdown on disk and rewrites `[[doc#oldSlug]] → [[doc#newSlug]]` via `updateWikiLinkAnchors`, preserving alias text. Mirrors the cross-vault propagation pattern of `propagateRename` for doc renames.
- ⚙️ **`brokenAnchorState` + `clearBrokenAnchorState()`** — exposed by `useLinkIndex`. When `updateDocumentLinks` detects a heading delete that leaves consuming wiki-links broken, populates `brokenAnchorState = { docPath, deletedIds, affectedRefs }` so the shell-level `BrokenAnchorBanner` can surface the affected refs and offer "Remove anchors" / "Leave broken" actions. `clearBrokenAnchorState()` dismisses the state.
...
- ⚙️ **YAML frontmatter parser (MVP-4a)** — first frontmatter parser in this codebase, dependency-free and intentionally constrained: a leading `---` fenced block at offset 0 is split off and parsed; only the flat-map `sources:` block list (`- url: …` / `  title: …`) is interpreted, every other key is preserved verbatim in `rawYaml` so unknown frontmatter round-trips through a sources edit. Inputs are normalised CRLF→LF and a leading UTF-8 BOM is stripped before parsing; non-leading `---` lines are not treated as frontmatter; an unclosed leading fence falls back to "no frontmatter" with the full text preserved as body. `parseFrontmatter` and `serializeFrontmatter` are a fixed point on their own outputs. `useDocumentContent` parses on load (exposing `sources` + `rawYaml` alongside the body), serialises on save, and provides `updateSources(next)` so a body-edit preserves frontmatter and a sources-edit preserves the body. (`features/document/utils/frontmatter.ts`, `features/document/hooks/useDocumentContent.ts`)
...
- ⚙️ **`useDocumentFileWatcher`** — subscribes to the `"content:doc"` `vault_change` event tick; compares `diskChecksumRef` to the current on-disk checksum on each event. If the file changed and the document is clean, silently reloads (records a "Reloaded from disk" history entry, moves the saved point, shows a toast). If the file changed and the document is dirty, exposes `conflictContent` so `DocumentView` can show a `ConflictBanner`; `handleKeepEdits` suppresses re-prompting for the same disk version via `dismissedChecksumRef`.
...
`features/document/utils/headerSlug.ts`, `features/document/utils/extractHeaders.ts`, `features/document/components/HeadingCopyLink.tsx`, `features/document/hooks/useLinkIndex.ts`, `shared/components/BrokenAnchorBanner.tsx`. Internal helpers cross-referenced from §1.4 (`PaneEntry.anchor`), §4.1 (`MarkdownPane.anchor`), §4.3 (`SluggedHeading`, `HeadingCopyLink`), §4.7 (`updateWikiLinkAnchors`, `stripWikiLinkAnchors`), §4.9 (`LinkIndexEntry.headers`, `findHeaderRename`, header rename auto-refactor, `brokenAnchorState`).
...
- ⚙️ **`headerSlug(text)`** (`features/document/utils/headerSlug.ts`) — canonical heading-slug helper: lowercases, de-accents, strips punctuation, collapses whitespace runs to single hyphens, trims leading/trailing hyphens. Idempotent and shared by the link index and the editor's heading renderer so a heading id, a `[[doc#section]]` anchor, and the BrokenAnchorBanner all agree on a single slug.
- ⚙️ **`extractHeaders(content)`** (`features/document/utils/extractHeaders.ts`) — parses ATX headings (H1–H6) from markdown into `{ id, text, level }[]`. Ignores hashes inside fenced or indented code blocks; trims trailing spaces and the optional ATX closing-hash run.
...
- ✅ **File-level source links** (MVP-4b) — `SourcesSection` mounted in a new `SvgProperties` aside. Sources persist in a lazily-created `<file>.svg.refs.json` sidecar (`infrastructure/svgRefsRepo.ts`); the sidecar is deleted from disk when sources and the reserved `attachedTo` field are both empty. Hook: `features/svgEditor/hooks/useSvgMeta.ts` — debounced 200 ms writes, file-switch flush, write-failure leaves `isDirty=true`.
...
- ✅ **`TabView`** (`src/app/knowledge_base/features/tab/TabView.tsx`) — pane shell mounting `TabToolbar` + `TabCanvas`; loading / canvas / engine-load-error chrome; theme push via `useObservedTheme()`; source-parse failures route to `ShellErrorContext`. Pane title is derived from the score's `\title` directive when present and falls back to the file's basename without extension via `paneTitleFor` (`src/app/knowledge_base/features/tab/paneTitle.ts`); the alphaTab `"Untitled"` sentinel is treated as fallback rather than as a real title.
...
- ✅ **File-level source links** — `SourcesSection` mounted at file scope in `TabProperties` (immediately after the FileReferences block). Sources persist in the existing `.alphatex.refs.json` sidecar (now `version: 3` — `domain/tabRefs.ts`). Hook: `features/tab/hooks/useTabSources.ts` — debounced 200 ms writes with read-modify-write merge guard so concurrent `sectionRefs`/`trackRefs` writes from `useTabEngine` are not clobbered.
- ⚙️ **Sidecar v3 migration** — `tabRefsRepo` reads v1, v2, and v3 (each migrated to v3 in memory); writes always emit v3 with empty `sources`/`attachedTo` arrays dropped. `reconcileSidecar*` helpers carry `sources`/`attachedTo` forward via conditional spread.
- ⚙️ **Forward-compat `attachedTo`** — both the SVG sidecar and the tab sidecar accept an optional `attachedTo: AttachedToEntry[]` field shared via `shared/types/attachments.ts`. No UI in MVP-4b binds this; reserved for the deferred MVP-2 SVG/Tab attachment work.
...
- ✅ **Skill bootstrap on launch** (MVP-3 Task 4) — on first chat-drawer open, `~/.claude/skills/knowledge-base/` is recursive-copied from the bundled resource if missing. Idempotent (re-launches no-op). One-shot toast appears on first install.
  ...
  - Backed by Rust `skill_status` + `skill_install_from_bundle` in `src-tauri/src/skill/`.
- ✅ **Skills sheet** (MVP-3 Task 6) — a "Skills" header button opens an in-drawer overlay listing every `/kb` subcommand as a card with a structured form. Argument-style cards take a text field; `/kb edit` and `/kb transform` open a `VaultFilePickerModal` scoped to file types.
  - `src/app/knowledge_base/features/claude/skills/SkillsSheet.tsx`
  - `src/app/knowledge_base/features/claude/skills/SkillCard.tsx`
- ✅ **Vault file picker modal** (MVP-3 Task 7) — a breadth-first walker over `tauriBridge.list(dir)` surfaces vault-relative file paths filtered by extension. Used by `/kb edit` (`.json`) and `/kb transform` (any). Skips dot-directories.
  - `src/app/knowledge_base/features/claude/skills/VaultFilePickerModal.tsx`
- ✅ **"Initialize with full template" splash + dropdown action** (MVP-3 Task 9) — `UninitializedVaultSplash` and `VaultSwitcher` both expose a third action that runs the basic init then auto-submits `/kb init` into the chat (drawer auto-opens).
  - `src/app/knowledge_base/shared/components/UninitializedVaultSplash.tsx`
  - `src/app/knowledge_base/shared/components/VaultSwitcher.tsx`
  - Wiring in `src/app/knowledge_base/knowledgeBase.tsx`
...
- ⚙️ **macOS Tauri debug build CI** (`.github/workflows/ci.yml` `tauri-build` job, MVP-1d) — `macos-latest` job that runs `npm run build` (static export) then `cargo build --manifest-path src-tauri/Cargo.toml` to verify the Rust shell compiles against the front-end bundle. Catches Tauri-compile regressions that the Node jobs can't see.
- ⚙️ **Static export** (`next.config.ts`) — `output: "export"` is unconditional; Next always emits `out/` with `trailingSlash: true` and `images.unoptimized: true`. This is permanent — Tauri's `frontendDist` points at `out/` and GitHub Pages is retired. `NEXT_PUBLIC_BASE_PATH` is exposed to client code for asset URL construction (manifest, icons, sw.js, soundfont).

---

## 13. External Contracts (for reference in test design)

- **File System Access API** — `showDirectoryPicker`, `FileSystemDirectoryHandle`, `FileSystemFileHandle`, `FileSystemWritableFileStream` (typed via TypeScript's built-in `lib.dom` — the project-local `types/file-system.d.ts` shim was retired in MVP-1d). Only supported in Chromium-family browsers; Tauri paths use the Rust VFS adapter instead.
