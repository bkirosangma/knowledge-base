...
14. **alphaTab Bravura font 404 in dev/playwright** — _Originally blocked by Bravura 404; the underlying font wiring is now resolved (Bravura files in `public/font/`, `FONT_DIRECTORY` constant in `alphaTabAssets.ts` consumed by `alphaTabEngine.ts:416`, all routed through `NEXT_PUBLIC_BASE_PATH`). With un-fixme'd, the test gets past the loading overlay AND the editor toolbar (Q/W/E/R/T/Y, V1/V2, technique groups) AND TabProperties (metadata-derived "Smoke" title + 120 BPM) all render._ **Remaining blocker:** `TabEditorCanvasOverlay` shows no cursor-target cells — metadata reaches TabProperties but `metadata.totalBeats` is apparently 0 (or null) when the overlay renders. Needs an investigation into the alphaTab "loaded" → metadata propagation path inside `useTabEngine.ts` (the "loaded" event might fire before `score.masterBars` is laid out, leaving `totalBeats` 0). Re-enable by: (1) probing metadata.totalBeats at "loaded" time, (2) if 0, wait for a follow-up event or recompute on first repaint. The `e2e/tabEditor.spec.ts` BLOCKED comment captures the diagnostic state.
