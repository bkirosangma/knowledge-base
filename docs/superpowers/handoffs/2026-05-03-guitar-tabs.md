...
14. **alphaTab Bravura font 404 in dev/playwright** — _Originally blocked by Bravura 404; the underlying font wiring is now resolved (Bravura files in `public/font/`, `FONT_DIRECTORY` constant in `alphaTabAssets.ts` consumed by `alphaTabEngine.ts:416`, all routed through `NEXT_PUBLIC_BASE_PATH`). With un-fixme'd, the test now gets past the loading overlay successfully._ **Remaining:** the body assertions still fail — the Edit toggle is now the PaneHeader "Exit Read Mode" button (already updated), but the cursor-target overlay query times out. Likely needs an extra wait between toggle and overlay query for `metadata` to populate after alphaTab finishes parsing. The updated BLOCKED comment in `e2e/tabEditor.spec.ts` documents the new staleness. Re-enable by walking the assertions against the post-PaneHeader/redesigned-toolbar/AudioWorklet UI.
