...
- ~~**Document-frontmatter inline-vs-block-list rule is documented but not enforced.**~~ _Partially closed: `kb_validate_doc.py` now warns (`fixable=True`) when frontmatter contains an inline `sources: [...]` list, pointing at `kb_transform.py --add-conventions`. The auto-rewrite hop in `kb_transform.py` is **not yet implemented** — current transform unconditionally strips frontmatter, so a sources-preserving rewrite path needs a coordinated decision with the project's TS frontmatter parser before it lands. The validation warning is the actionable artifact today._
