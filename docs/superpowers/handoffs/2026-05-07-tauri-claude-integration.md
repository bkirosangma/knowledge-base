...
**Last updated:** 2026-05-09 (MVP-3 merged via PR #155 (`726904b` on `main`), MVP-3.5 merged via PR #156 (`678a9d8` on `main`), and MVP-4 merged via PR #157 (`072f807` on `main`).
...
**MVP-4.x carved out** during MVP-4 ship (post-Task 18) — `tauri-plugin-webdriver 0.2.1` failed to bind `:4444` in four different headless-CI configurations (macOS WKWebView; Ubuntu WebKitGTK + xvfb; +dbus; +AT-SPI). Plugin appears broken at runtime, not at the env layer. The e2e job was dropped from CI to ship MVP-4; landing real automated e2e (no manual smoke) is the goal of MVP-4.x. See Open follow-up items for the diagnostic dump and Next Action for the plan-writing brief.

Post-merge cleanup run; `feat/tauri-mvp4x-real-e2e-ci` cut from `main`. Next action: write the MVP-4.x plan (deep investigation: plugin source → official `tauri-driver` → Option C custom IPC harness → last-resort self-hosted macOS) and dispatch via subagents.
...
| **MVP-4.x** | Real Playwright e2e in CI (no manual smoke) | _not yet written; due before MVP-5_ | 🚧 Active on `feat/tauri-mvp4x-real-e2e-ci`; plan pending. `tauri-plugin-webdriver 0.2.1` failed to bind in 4 headless configs — see follow-ups for diagnostics. |
...
- **Phase 1 outcome — port-mismatch root cause confirmed (2026-05-09 21:24, on `feat/tauri-mvp4x-real-e2e-ci`).** The plugin's bind site is `tauri-plugin-webdriver-0.2.1/src/server/mod.rs:69` (`tokio::net::TcpListener::bind(addr)` inside a `std::thread::spawn`-owned tokio runtime), driven by `DEFAULT_PORT = 4445` in `src/lib.rs:19`. **The bind never failed in any of the four CI configs — it succeeded silently on `:4445` while Playwright's `webServer.url` polled `:4444` for 180 s.** Local repro (`npm run tauri:dev`, macOS host): `curl http://localhost:4445/status` returned `{"value":{"message":"tauri-plugin-webdriver is ready","ready":true}}` within 2 s, `:4444` was correctly absent, `lsof` confirmed the listener on `127.0.0.1:4445`. The "no logs" symptom is a separate (orthogonal) issue: the plugin uses `tracing::info!` and Tauri 2.11.1 does not install a `tracing-subscriber` by default, so `RUST_LOG=tauri_plugin_webdriver=trace` produces no output regardless of bind state. **Fix:** swap `tauri_plugin_webdriver::init()` → `tauri_plugin_webdriver::init_with_port(4444)` in `src-tauri/src/main.rs` (the Playwright-side `:4444` contract is the source of truth; the plugin's port default is incidental). Verified locally: `curl http://localhost:4444/status` returns the ready response within 2 s after the change. Docker/Ubuntu repro deferred to Phase 3.A's first CI push per plan allowance — the bug is platform-independent (single-line port number, no platform-specific code path), so a Docker pass would only reconfirm what the local pass already proves; the real CI runners will validate. **Recommendation: proceed to Phase 3.A** (verified fix; skip Phase 2's `tauri-driver` evaluation entirely).
...
**MVP-4 merged via PR #157 (`072f807` on `main`).** Post-merge cleanup done; `feat/tauri-mvp4x-real-e2e-ci` cut from `main`; this doc is being updated on that branch alongside the first MVP-4.x commit. **MVP-4.x kicks off now** — write the plan to land real automated Playwright e2e in CI with no manual smoke. Skip MVP-5 until MVP-4.x ships.

**Bootstrap (already run for this branch):**

```bash
cd "/Users/kiro/My Projects/knowledge-base"
git checkout main && git pull --ff-only            # 072f807 present
git branch -D feat/tauri-mvp4-test-infra           # remote auto-deleted; [origin/...: gone] verified
git remote prune origin
git checkout -b feat/tauri-mvp4x-real-e2e-ci       # ← you are here
```

**MVP-4.x plan-writing brief (run via `superpowers:writing-plans`; output `docs/superpowers/plans/2026-05-09-tauri-mvp4x-real-e2e-ci-plan.md`):**
