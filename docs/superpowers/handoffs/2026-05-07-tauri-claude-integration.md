...
| **MVP-4** | Test infrastructure on the new shell | _not yet written; due now on `feat/tauri-mvp4-test-infra`_ | 🚧 Branch cut; plan pending. |
...
- **Sequence the plan: trait + stub first, infra second, e2e last.** Suggested sequencing:
  ...
  4. Vitest Tauri-bridge contract layer — port the FSA-shaped `*Repo.test.ts` files in `infrastructure/` to `*RepoTauri.test.ts` against a mocked `@tauri-apps/api/core` `invoke()`. Migration sweep — boring but mechanical.
  5. `tauri-plugin-webdriver` wiring — dev-only dependency behind `#[cfg(debug_assertions)]` per spec § 9.3. `playwright.config.ts` gains a `webdriver` project that boots `cargo tauri dev` with `KB_CLAUDE_MODE=stub`, waits for the WebDriver port at `localhost:4444`, runs the e2e specs.
...
- **Plan-level decisions to pin explicitly:**
  ...
  - **Terminal surface in the e2e proof set** — does MVP-4 ship a terminal e2e? **Default recommendation:** yes, but stub-side: `term_event` stream replay through the existing `useTerminalSession` listener path. A real-PTY e2e adds host-side spawn complexity that's better handled inside the existing `cargo test` boundary (Rust integration tests under `src-tauri/tests/`) than through Playwright. Cite "real-PTY through tauri-plugin-webdriver" as the alternative; defer if it adds CI flakiness.
