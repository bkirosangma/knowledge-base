...
**Last updated:** 2026-05-10 (MVP-3 merged via PR #155 (`726904b` on `main`), MVP-3.5 merged via PR #156 (`678a9d8` on `main`), MVP-4 merged via PR #157 (`072f807` on `main`), MVP-4.x merged via PR #158 (`6903077` on `main`), and MVP-5 ready-for-review on PR #159 (test-cases sweep complete on `feat/tauri-mvp5-test-promotion`).
...
| **MVP-5** | Promote previously-blocked test cases | `docs/superpowers/plans/2026-05-09-tauri-mvp5-test-promotion-plan.md` | ✅ Ready-for-review on PR #159 (`feat/tauri-mvp5-test-promotion`). 74 ❌ cases disposed: 43 promote (38 e2e + 5 unit) + 1 ✅-flip + 4 retire + 26 defer with `note:` markers. 12 new e2e spec files (38 specs total green in 30.5s). 4 modified Vitest files. 4 new fixture vaults. No production-code changes; no `dispatch.rs` expansion. |
...
**MVP-5 ready-for-review on PR #159.** Once it merges, the **Tauri + Claude Integration epic is *done*** — the original spec (vault-on-disk, Claude subprocess, skill bootstrap, embedded terminal, real e2e in CI, test-cases sweep) has all shipped across MVPs 1a–5. No further "core epic" work is required for this handoff to retire.

**Residual:** 26 ❌ test cases carry forward as MVP-6+ candidates. Each carries an inline `note: see MVP-5 follow-up — <reason>` annotation in `test-cases/` so they're self-describing. Themes: PWA harness, `test_server` `dispatch.rs` expansion for AppHandle-dependent commands (settings_*, term_*, claude_*, skill_*), production code changes to expose editor refs for assertions, alphaTab canvas-rendering fragility, DnD harness for explorer drag flows. **No new MVP-6 plan is required for the residuals to ship** — they're discrete follow-ups that can land independently as small PRs against `main`, each one:

1. Picking a single ❌ case (or small group sharing a blocker) from `test-cases/`.
2. Removing the `note:` annotation and either promoting or retiring per the actual blocker.
3. Shipping with the case-flip in the same commit as the test that covers it (per `test-cases/README.md`'s "flip the status marker in the same commit" rule).

**Bootstrap for any follow-up MVP-6.x PR:**

```bash
cd "/Users/kiro/My Projects/knowledge-base"
git checkout main && git pull --ff-only          # PR #159 should be merged before this
git checkout -b feat/<descriptive-name>          # e2e/dispatch-handle-expansion, e2e/pwa-harness, etc.
# Pick from test-cases/*.md the cases that share a blocker, lift the note: annotation,
# write the spec or unit test, flip the marker, ship.
```

**If a future MVP needs the test_server's AppHandle command surface expanded** (the largest residual block — covers ~10 of the 26 deferred cases including settings/skills/claude/terminal flows), that work *would* need a small plan since `dispatch.rs` will need a real `AppHandle` factory. But it's still a discrete MVP, not a continuation of this epic. Spin up a fresh handoff doc when that work begins.
